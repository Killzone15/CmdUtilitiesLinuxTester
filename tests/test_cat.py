...
def test_display_file(temp_file):
    cat_util = CatUtility(temp_file)
    expected_output = 'Line 1\nLine 2\nLine 3\n'
    # Проверяем, что вывод соответствует ожидаемому
    assert cat_util.run_command(['cat', temp_file]) == expected_output


def test_display_multiple_files(temp_files):
    cat_util = CatUtility()
    expected_output = (
        'File 1 Line 1\n'
        'File 1 Line 2\n'
    ...
    assert cat_util.run_command(['cat'] + temp_files) == expected_output
...
def test_display_file_with_options(temp_file, options, expected_output):
    cat_util = CatUtility(temp_file)
    assert cat_util.run_command(['cat'] + options + [temp_file]) == expected_output
